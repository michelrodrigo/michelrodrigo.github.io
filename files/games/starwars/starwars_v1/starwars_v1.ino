...
#define INTERVALO_PISCA_LED 200
...
int seletor_A[] = {LOW, LOW, HIGH, HIGH, LOW, HIGH, LOW, HIGH};
...
void setup(){
  ...
  evento_led = t.every(50, atualizaSaida);
  evento_teste = t.every(500, teste1);
  evento_contador = t.every(515, atualizaContador);
...
void teste3(){
  ...
  while(rand_num1 == rand_num2){
   ...
   evento_le_entrada = t.every(20, leEntrada);
   evento_desliga_entrada = t.after(4000, desligaEntrada);
...
//  Serial.println(saida[2], BIN);
  ...
  if(j1_acertou && !timer_led1_setado){
    ...
    timer_led1_setado = true;
      evento_buzzer = t.oscillate(BUZZER, INTERVALO_PISCA_LED, LOW, 1);
  }
