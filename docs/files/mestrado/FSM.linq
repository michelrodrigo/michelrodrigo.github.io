...
var proj_sup = sup.Projection(sup.UncontrollableEvents);
...
quantEspecification(out var EqA, "61", 1);
quantEspecification(out var EqB, "63", 1);
quantEspecification(out var EqC, "65", 2);

var Eq = EqA.ParallelCompositionWith(EqB).ParallelCompositionWith(EqC);
...
void FSM(out List<DeterministicFiniteAutomaton> p, out List<DeterministicFiniteAutomaton> spec)
{
	...
	p = new[] {c1, c2, milling, lathe, robot, mm, c3, mp}.ToList();
	spec = new[] {e1, e2, e3, e4, e5, e6, e78}.ToList();
}
...
double sequenceCount2(DeterministicFiniteAutomaton g)
{
	...
	//gets a list with all transitions of g
	var tr = g.Transitions.ToList();

	//creates a list that stores transitions to be removed from tr
	...
	//creates a list that stores origin states for the transitions
